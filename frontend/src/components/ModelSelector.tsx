...
const BADGE_COLORS: Record<string, string> = {
  ...
  Standard:
    "bg-brand-blue-light/20 text-brand-blue-dark border-brand-blue-light/30",
...
export function ModelSelector({
  ...
  if (loading || !selected) {
    return (
      <div className="h-8 w-40 rounded-lg bg-brand-navy-light animate-pulse" />
    );
  }
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 rounded-xl bg-white border border-brand-gray shadow-xl shadow-brand-navy/10 z-50 overflow-hidden">
          <div className="px-4 py-2.5 border-b border-brand-gray bg-brand-surface-muted">
            <p className="text-xs font-semibold text-brand-gray-text uppercase tracking-wider">
              Choisir le modèle
            </p>
          </div>
                  ...
                  <div className="flex items-center gap-2 px-2 py-1.5">
                    ...
                    <span
                      className={`ml-auto text-[10px] px-1.5 py-0.5 rounded border font-medium ${
                        ...
                        "bg-brand-gray text-brand-gray-text border-brand-gray-mid"
                      }`}
                    >
                      {provider.badge}
                    </span>
                  ...
                  <div className="space-y-0.5">
                    {provider.models.map((m) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`w-2 h-2 rounded-full flex-shrink-0 ${isActive ? "bg-brand-blue" : "bg-brand-gray-mid"}`}
                          />
                          <div className="flex-1 min-w-0">
                            <p
                              className={`text-sm font-medium ${isActive ? "text-brand-navy" : "text-brand-navy/80"}`}
                            >
                              {m.name}
                            </p>
