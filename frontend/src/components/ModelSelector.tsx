...
const BADGE_COLORS: Record<string, string> = {
  ...
  Fastest:  "bg-cyan-500/20 text-cyan-400 border-cyan-500/30",
  Powerful: "bg-brand-purple/20 text-brand-purple-light border-brand-purple/30",
  Standard: "bg-blue-500/20 text-blue-400 border-blue-500/30",
  Private:  "bg-amber-500/20 text-amber-400 border-amber-500/30",
  Auto:     "bg-gradient-to-r from-cyan-500/20 to-purple-500/20 text-white border-cyan-500/30",
};
...
export function ModelSelector({ providers, selected, onSelect, loading }: Props) {
  ...
  if (loading || !selected) {
    return <div className="h-8 w-40 rounded-lg bg-brand-gray animate-pulse" />;
  }
  ...
  return (
    <div ref={ref} className="relative">
      ...
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-brand-gray hover:bg-brand-gray-light border border-brand-gray-light/50 transition-colors text-sm"
      >
      ...
      {/* Dropdown panel */}
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 rounded-xl bg-brand-surface border border-brand-gray shadow-2xl shadow-black/50 z-50 overflow-hidden">
          <div className="px-4 py-2.5 border-b border-brand-gray">
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Select Model</p>
          </div>
          ...
          <div className="max-h-96 overflow-y-auto p-2 space-y-1">
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left transition-colors mb-2 ${
                autoIsActive
                  ...
                  : "hover:bg-brand-gray border border-transparent"
              }`}
            >
              <span className="text-lg leading-none">✦</span>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-white">Auto</p>
                <p className="text-xs text-gray-400">Picks the best available model automatically</p>
              </div>
              {autoIsActive && (
            ...
            <div className="border-t border-brand-gray/50 mb-2" />
                ...
                <div className="flex items-center gap-2 px-2 py-1.5">
                  ...
                  <span
                    className={`ml-auto text-[10px] px-1.5 py-0.5 rounded border font-medium ${
                      BADGE_COLORS[provider.badge] ?? "bg-gray-500/20 text-gray-400 border-gray-500/30"
                    }`}
                  >
                    {provider.badge}
                  </span>
                ...
                <div className="space-y-0.5">
                  {provider.models.map((m) => {
                    ...
                    return (
                      ...
                      >
                        <div className={`w-2 h-2 rounded-full flex-shrink-0 ${isActive ? "bg-brand-purple" : "bg-brand-gray-light"}`} />
                        <div className="flex-1 min-w-0">
                          <p className={`text-sm font-medium ${isActive ? "text-white" : "text-gray-200"}`}>{m.name}</p>
