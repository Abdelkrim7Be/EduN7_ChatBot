    ...
    <div className="p-6 max-w-6xl mx-auto space-y-5">
      <div>
        ...
        <p className="text-sm text-brand-gray-text mt-0.5">
          ...
          partagé{counts.shared !== 1 ? "s" : ""}, {counts.private} privé
          {counts.private !== 1 ? "s" : ""}
        </p>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-brand-gray">
              {filtered.map((doc) => (
                <tr
                  key={doc.doc_id}
                  className="hover:bg-brand-surface-muted transition-colors group"
                >
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div className="min-w-0">
                        <p
                          className="font-medium text-brand-navy truncate max-w-[200px]"
                          title={doc.name}
                        >
                          {doc.name}
                        </p>
                  ...
                  <td className="px-4 py-3 hidden md:table-cell">
                    <span
                      className={`text-xs font-medium px-2 py-0.5 rounded-full ${CATEGORY_COLORS[doc.category] ?? CATEGORY_COLORS["Autres"]}`}
                    >
                      {doc.category}
                    </span>
                  </td>
              ...
              {filtered.length === 0 && (
                <tr>
                  <td
                    colSpan={7}
                    className="px-5 py-12 text-center text-sm text-brand-gray-text"
                  >
