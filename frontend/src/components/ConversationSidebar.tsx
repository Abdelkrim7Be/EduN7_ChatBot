...
const CATEGORY_ORDER = ["Cours", "TD / TP", "Examens", "Projets", "Corrections", "Autres"];
...
function groupByCategory(docs: DocumentRecord[]): { category: string; docs: DocumentRecord[] }[] {
...
function RenameInput({
  ...
  return (
    <input
      ...
      className="w-full bg-transparent border-b border-brand-gold text-white text-xs outline-none py-0.5"
    ...
    <div
      ...
      className={`group relative flex items-start gap-2 px-3 py-2 rounded-lg cursor-pointer transition-colors ${
        isActive
          ...
          : "hover:bg-brand-navy-light border-l-2 border-transparent"
      }`}
    >
      <div className="flex-1 min-w-0">
        {renaming ? (
          ...
          <p className={`text-xs font-medium truncate leading-snug ${isActive ? "text-white" : "text-white/80"}`}>
            {conv.title}
          </p>
        )}
      ...
      <button
        ...
        title="Supprimer"
        className="opacity-0 group-hover:opacity-100 flex-shrink-0 p-0.5 text-white/30 hover:text-red-400 transition-all mt-0.5"
      >
        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export function ConversationSidebar({
  ...
  if (collapsed) {
    return (
      <aside className="w-10 flex-shrink-0 bg-brand-navy border-r border-brand-navy-border flex flex-col items-center py-3 gap-3">
        <button
          onClick={onCollapseToggle}
          title="Afficher le panneau"
          className="p-1.5 rounded-lg hover:bg-brand-navy-light text-white/40 hover:text-white transition-colors"
        >
        ...
        {conversations.length > 0 && (
          <span className="text-[10px] font-bold text-white/25">{conversations.length}</span>
        )}
      </aside>
    );
  }
  ...
  return (
    <aside className="w-72 flex-shrink-0 bg-brand-navy border-r border-brand-navy-border flex flex-col">

      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-brand-navy-border flex-shrink-0">
        <span className="text-sm font-semibold text-white">Conversations</span>
        <button
          onClick={onCollapseToggle}
          title="Réduire le panneau"
          className="p-1 rounded hover:bg-brand-navy-light text-white/30 hover:text-white/80 transition-colors"
        >
      ...
      <div className="px-3 pt-2 flex-shrink-0">
        <div className="relative">
          <svg className="w-3.5 h-3.5 absolute left-2.5 top-1/2 -translate-y-1/2 text-white/25" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <input
            ...
            className="w-full bg-brand-navy-light border border-brand-navy-border text-white text-xs placeholder-white/25 rounded-lg pl-8 pr-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-brand-blue/40"
          />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-2 space-y-1 min-h-0">
        {conversations.length === 0 ? (
          <p className="text-xs text-white/30 text-center pt-6 px-4">
            Aucune conversation. Démarrez un nouveau chat !
          </p>
        ) : (
          groups.map(({ label, items }) => (
            <div key={label} className="mb-1">
              <p className="px-2 py-1 text-[10px] font-semibold uppercase tracking-wider text-white/25">
                {label}
              </p>
              {items.map((conv) => (
                <ConvItem
      ...
      {/* Context / Documents */}
      <div className="border-t border-brand-navy-border flex-shrink-0">
        ...
        <div className="max-h-64 overflow-y-auto px-2 pb-3 space-y-2">
          {documents.length === 0 ? (
            ...
            groupByCategory(documents).map(({ category, docs }) => {
              const style = CATEGORY_STYLE[category] ?? CATEGORY_STYLE["Autres"];
              const allSelected = docs.every((d) => selectedDocIds.has(d.doc_id));
              const someSelected = docs.some((d) => selectedDocIds.has(d.doc_id));
              return (
                <div key={category}>
                  {/* Category header */}
                  <button
                    onClick={() => docs.forEach((d) => {
                      if (allSelected ? selectedDocIds.has(d.doc_id) : !selectedDocIds.has(d.doc_id))
                        onToggleDoc(d.doc_id);
                    })}
                    className="w-full flex items-center gap-1.5 px-1 py-1 rounded hover:bg-brand-navy-light transition-colors group/cat"
                  ...
                  >
                    ...
                    <span className="text-[10px] text-white/20 ml-auto">
                      {someSelected ? `${docs.filter((d) => selectedDocIds.has(d.doc_id)).length}/${docs.length}` : docs.length}
                    </span>
                    <span className="text-[10px] text-white/20 opacity-0 group-hover/cat:opacity-100 transition-opacity">
                      {allSelected ? "−" : "+"}
                    </span>
                  </button>
                  ...
                  <div className="space-y-0.5 pl-1">
                    {docs.map((doc) => (
                      <div
                        key={doc.doc_id}
                        className="flex items-start gap-2 p-1.5 rounded-lg hover:bg-brand-navy-light group"
                      >
                        <input
                          type="checkbox"
                          checked={selectedDocIds.has(doc.doc_id)}
                          onChange={() => onToggleDoc(doc.doc_id)}
                          className="mt-0.5 accent-brand-gold cursor-pointer flex-shrink-0"
                        />
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1 min-w-0">
                            <p className="text-xs text-white/80 font-medium truncate" title={doc.name}>
                              {doc.name}
                            </p>
                            {doc.scope === "shared" && (
                        ...
                        <button
                          ...
                          title="Supprimer"
                          className="opacity-0 group-hover:opacity-100 p-0.5 text-white/30 hover:text-red-400 transition-all flex-shrink-0 mt-0.5"
                        >
                          <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                          </svg>
